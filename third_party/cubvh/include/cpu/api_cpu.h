...
static py::array_t<int> fill_holes(
    ...
    if (return_added) {
        ...
        py::array_t<int> out({(py::size_t)added.size(), (py::size_t)3});
        ...
        py::array_t<int> out({(py::size_t)F.size(), (py::size_t)3});
        auto obuf = out.request();
        int* optr = static_cast<int*>(obuf.ptr);
...
static std::pair<py::array_t<float>, py::array_t<int>> merge_vertices(
    ...
    py::array_t<float> v_out({(py::size_t)V_out.size(), (py::size_t)3});
    py::array_t<int> f_out({(py::size_t)F_out.size(), (py::size_t)3});
...
static std::pair<py::array_t<float>, py::array_t<int>> sparse_marching_cubes_cpu(
    ...
    // Allocate outputs
    py::array_t<float> v_out({(py::size_t)V.size(), (py::size_t)3});
    py::array_t<int>   f_out({(py::size_t)F.size(), (py::size_t)3});
...
{
    py::array_t<T> V({ (size_t)m.vertices.size(), (size_t)3 });
    py::array_t<int> F({ (size_t)m.faces.size(), (size_t)3 });
    auto vbuf = V.template mutable_unchecked<2>();
    for (size_t i = 0; i < (size_t)m.vertices.size(); ++i) {
        vbuf(i,0) = m.vertices[i].x;
        vbuf(i,1) = m.vertices[i].y;
        vbuf(i,2) = m.vertices[i].z;
    }
    auto fbuf = F.template mutable_unchecked<2>();
    for (size_t i = 0; i < (size_t)m.faces.size(); ++i) {
